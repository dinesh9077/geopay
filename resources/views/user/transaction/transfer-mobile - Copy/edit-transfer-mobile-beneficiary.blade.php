<div class="modal fade" id="editTransferMobileBeneficiary" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
	<div class="modal-dialog modal-dialog-centered modal-dialog-scrollable modal-fullscreen-lg-down modal-xl">
		<div class="modal-content">
			<div class="modal-header">
				...
				<button type="button" class="btn-close" data-bs-dismiss="modal" onclick="$('#editTransferMobileBeneficiary, .modal-backdrop').remove();" aria-label="Close"></button>
			</div>
				...
				<form id="editTransferMobileBeneficiaryForm" method="post" action="{{ route('transfer-to-mobile.beneficiary-update', ['id' => $beneficiary->id]) }}"> 
					<div class="row">
						...
						<div class="mb-4 col-md-6">
							...
							<input id="recipient_address" name="recipient_address" placeholder="(Apt/Street/Area/Zip code)" type="text" class="form-control form-control-lg content-3" value="{{ $edit['recipient_address'] ?? '' }}" />
						</div>
			...
			<div class="modal-footer">
				<button type="button" class="btn btn-secondary" data-bs-dismiss="modal" onclick="$('#editTransferMobileBeneficiary, .modal-backdrop').remove();">Close</button>
	...
	<script> 
		...
		$beneficiaryForm.find('#recipient_country').change(function() {
			...
			const channelId = @json($edit['channel_id'] ?? 0) || ''; 
