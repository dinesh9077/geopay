		...
		<div class="card-body"> 
			<form class="forms-sample row" id="onafricForm" action="{{ route('admin.third-party-key.update') }}?module_type=onafric_mobile_setting" method="post" enctype="multipart/form-data">
				...
				{{-- <div class="mb-3 col-md-6">
					...
					<select class="form-control" id="onafric_commission_type" name="onafric_commission_type" > 
						<option value="flat" {{ config('setting.onafric_commission_type') == "flat" ? 'selected' : '' }}>Flat/Fix</option>
						<option value="percentage" {{ config('setting.onafric_commission_type') == "percentage" ? 'selected' : '' }}>Percentage</option>
					</select>
				</div>  
				<div class="mb-3 col-md-6">
