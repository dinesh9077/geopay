	...
	class ExchangeRateController extends Controller
	{
		...
		{ 
			...
			try
			{
				...
				foreach ($data as $row) 
				{
					$manualExchangeRate = ExchangeRate::updateOrCreate(
						[
					...
					// Recalculate merchant rates only for type = 2
					if ($manualExchangeRate->merchantRates && $row['type'] == 2) 
					{
						$rate = $row['aggregator_rate'];

						foreach ($manualExchangeRate->merchantRates as $merchantRate) 
						{
			...
			foreach ($lightnetCountries as $lightnetCountry) 
			{
				...
				LiveExchangeRate::updateOrCreate(
					...
					[
						...
						'api_markdown_rate' => 0, 
						'api_markdown_type' => "flat",
						'api_markdown_charge' => 0,
						'status' => 1,
